...
shl bx, 1
shl bx, 1
shl bx, 1 											; making bx = bx*32 for pcb
...
pop ax 											; read original bx from stack //because bx is used here for traversal
mov [pcb+bx+2], ax	 							; save bx in current pcb		// popping because pushed in start of function
...
mov [current], bx									 ; update current to new pcb	// updating current with next pcb
mov cl, 5
shl bx, cl 										; multiply by 32 for pcb start // bx = bx * 32
...
mov al, 0x20									;// closing timer interrupt
...
mov bx, [bp+4] 			; read next available pcb index // next pcb number
cmp bx, 2				; are all PCBs used			// one task being multitasked and one in main process
je multiexit 				; yes, exit
mov cl, 5
shl bx, cl 				; multiply by 32 for pcb start	// start of that pcb index starting with 0
mov ax, [bp+8] 			; read segment parameter		// cs
mov [pcb+bx+18], ax	 	; save in pcb space for cs		
mov ax, [bp+6] 			; read offset parameter		// ip of my task one

mov [pcb+bx+16], ax			 	; save in pcb space for ip
mov [pcb+bx+22], ds 			; set stack to our segment
mov si, [bp+4] 					; read this pcb index	// again reading in si for STACK
mov cl, 9
shl si, cl 						; multiply by 512			// for stack
add si, 256*2+stack 				; end of stack for this thread	// stack in upside down staring form last
sub si, 2					 	; decrement thread stack pointer	// first index from that end // below
mov [pcb+bx+14], si					 	; save si in pcb space for sp
mov word [pcb+bx+26], 0x0200				 ; initialize thread flags	// initillizing flags
mov ax, [pcb+28] 							; read next of 0th thread in ax
mov [pcb+bx+28], ax 						; set as next of new thread		// working as a circular chain adding new task into the circle
mov ax, [bp+4] 								; read new thread index
mov [pcb+28], ax							 ; set as next of 0th thread 	// setting as first thread
add word [bp+4], 1							; inc by 1
...
mov di,0
loopstarsall:
mov word[es:di],		0x01fe
mov word[es:di+4],		0x02fe
mov word[es:di+8],		0x03fe
mov word[es:di+12],	0x04fe
mov word[es:di+16],	0x05fe
mov word[es:di+20],	0x06fe
mov word[es:di+24],	0x07fe
mov word[es:di+28],	0x08fe
...
repeatkeyexitintro:
		...
		jmp repeatkeyexitintro	; else take key agian
